...
jsonDecAlbumList =
   ("listTitle" := Json.Decode.string) >>= \plistTitle ->
   ("childFirst" := jsonDecAlbumOrList) >>= \pchildFirst ->
   ("childRest" := Json.Decode.list (jsonDecAlbumOrList)) >>= \pchildRest ->
   ("listThumbnail" := jsonDecImage) >>= \plistThumbnail ->
...
jsonEncAlbumOrList  val =
    let keyval v = case v of
                    List v1 -> ("List", encodeValue (jsonEncAlbumList v1))
